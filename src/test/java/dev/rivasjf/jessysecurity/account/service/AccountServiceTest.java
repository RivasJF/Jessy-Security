...
public class AccountServiceTest {
    ...
    @Test
    void deleteAccount_ShouldNotReturn() {
        UUID accountId = UUID.randomUUID();
        UUID passwordInfoId = UUID.randomUUID();
        UUID emailInfoId = UUID.randomUUID();
        ...
        when(userRepository.findByEmail("jon@gmail.com")).thenReturn(java.util.Optional.of(fakeUser));
        when(accountRepository.deleteByUserAndPublicId(fakeUser, accountId)).thenReturn(java.util.Optional.of(fakeAccount));
        doNothing().when(accountRepository).delete(fakeAccount);


        assertDoesNotThrow(() -> accountService.deleteAccount("jon@gmail.com", accountId.toString()));
        verify(accountRepository, times(1)).deleteByUserAndPublicId(fakeUser, accountId);

    }

    @Test
    void deleteAccount_ShouldThrowIllegalArgumentException_WhenAccountNotFound() {
        UUID accountId = UUID.randomUUID();
        UUID passwordInfoId = UUID.randomUUID();
        UUID emailInfoId = UUID.randomUUID();
        ...
        EntityNotFoundException exception = assertThrows(
                ...
                () -> accountService.deleteAccount("jon@gmail.com", UUID.randomUUID().toString()),
                "Se esperaba un EntityNotFoundException pero no se lanzó"
        );
